    ...
    <div className="card task-card" data-status={task.status}>
      ...
      <div className="task-card-body">
        ...
        <div className="task-card-info">
          <span>任务量：<strong>{task.quantity}</strong></span>
          <span>基础积分：<strong className="points">{task.basePoints}</strong></span>
        </div>

        {taskType === 'periodic' && task.weekdays && task.weekdays.length > 0 && (
          <div className="task-card-weekdays">
            {getWeekdaysText()}
          </div>
        )}
        ...
        {isApproved && task.rating && (
          <div className="task-rating-box">
            ...
            <span className="task-rating-points">
              获得 {task.finalPoints} 积分
            </span>
          </div>
        )}
