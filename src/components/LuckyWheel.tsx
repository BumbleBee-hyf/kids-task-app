...
// 抽奖规则
const LOTTERY_RULES = ['每次抽奖消耗10积分', '奖品随机抽取，概率公平公正', '中奖后自动发放代金券']
...
export default function LuckyWheel({
  ...
  useEffect(() => {
    ...
    prizes.forEach((prize, i) => {
      ...
      ctx.font =
        prize.amount === 100 ? 'bold 15px Nunito, sans-serif' : 'bold 13px Nunito, sans-serif'
  ...
  return (
    <div className={styles.wheelGameContainer}>
      ...
      <div className={styles.wheelSideInfo}>
        <div className={styles.playsLeft}></div>
      </div>
      ...
      <div className={styles.wheelRules}>
        <div className={styles.wheelRulesTitle}>⭐ 抽奖规则</div>
        <ul className={styles.wheelRulesList}>
          {LOTTERY_RULES.map((rule, i) => (
            <li key={i} className={styles.wheelRulesItem}>
              {rule}
            </li>
          ))}
        </ul>
