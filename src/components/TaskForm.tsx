...
export default function TaskForm({
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (taskType === 'periodic' && weekdays.length === 0) {
      setError('请选择周期日期')
      return
    }
  ...
  return (
    <form className="task-form card" onSubmit={handleSubmit}>
      <h3 className="form-title">{initialData ? '✏️ 编辑任务' : '📝 发布新任务'}</h3>
        ...
        <div className="task-type-selector">
          <label className={`task-type-option ${taskType === 'temporary' ? 'active' : ''}`}>
            <input
              ...
              name="taskType"
              value="temporary"
              checked={taskType === 'temporary'}
              onChange={() => setTaskType('temporary')}
            />
          ...
          <label className={`task-type-option ${taskType === 'periodic' ? 'active' : ''}`}>
            <input
              ...
              name="taskType"
              value="periodic"
              checked={taskType === 'periodic'}
              onChange={() => setTaskType('periodic')}
            />
