...
const PRIZE_COLORS = ['#FFD700', '#4ADE80', '#38BDF8', '#FB923C', '#A78BFA', '#FF2D55', '#2DD4BF', '#818CF8', '#FBBF24', '#FCD34D'];

function PrizeEditor({ prizes, onChange, title }: {
  ...
  const add = () => {
    ...
    onChange([...prizes, { amount: 1, weight: 10, type: 'money', label: '新奖品', color: PRIZE_COLORS[colorIndex] }]);
  };
          ...
          <tbody>
            {prizes.map((p, i) => (
              <tr key={i} style={{ borderBottom: '1px solid var(--border-light)' }}>
                ...
                <td style={tdStyle}>
                  <input
                    type="number" value={p.amount} min={0}
                ...
                <td style={tdStyle}>
                  <input
                    type="number" value={p.weight} min={0.1} step={0.5}
                ...
                <td style={tdStyle}>
                  <button
                    ...
                    style={{ background: 'none', border: 'none', color: 'var(--color-danger)', cursor: prizes.length <= 1 ? 'not-allowed' : 'pointer', fontSize: '1.1rem', opacity: prizes.length <= 1 ? 0.3 : 1 }}
                  >
                    ✕
                  </button>
                </td>
...
const thStyle: React.CSSProperties = { padding: '8px 6px', textAlign: 'left', fontWeight: 600, fontSize: '0.8rem', color: 'var(--text-secondary)' };
const tdStyle: React.CSSProperties = { padding: '6px' };
const inputStyle: React.CSSProperties = { width: '100%', padding: '4px 8px', border: '1px solid var(--border-light)', borderRadius: 6, fontSize: '0.9rem', boxSizing: 'border-box', background: '#FFF8F0', color: '#2D2A26' };
const linkBtnStyle: React.CSSProperties = { background: 'none', border: 'none', color: 'var(--color-primary)', cursor: 'pointer', fontSize: '0.9rem', padding: 0 };
...
export default function LotteryConfigPage() {
  ...
  return (
    <div className="page-container">
      ...
      {message && (
        <div style={{
          padding: '10px 16px', borderRadius: 8, marginBottom: 'var(--spacing-md)',
          background: message.type === 'success' ? 'var(--color-success-bg, #dcfce7)' : 'var(--color-danger-bg, #fef2f2)',
      ...
      <div className="card" style={{ marginBottom: 'var(--spacing-lg)' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          <span>每次抽奖消耗</span>
          <input
            type="number" value={pointCost} min={1}
      ...
      <PrizeEditor
        ...
        onChange={wheelSegments => setConfig(prev => prev ? { ...prev, wheelSegments } : prev)}
      />

      {/* 操作按钮 */}
      <div style={{ display: 'flex', gap: 12, justifyContent: 'flex-end', marginTop: 'var(--spacing-md)' }}>
