...
export default function StudentDashboard() {
  ...
  // 签到按钮状态
  const checkinDisabled = checkinStatus.checkedInToday || !checkinStatus.hasCompletedTask || checkinLoading;
  ...
  return (
    <div className="page-container">
      ...
      <div className="welcome-card">
        ...
        <div className="welcome-checkin">
          ...
          <div className="checkin-text">
            ...
            <div className="checkin-days">{checkinStatus.streak}<span> 天</span></div>
          </div>
      ...
      <div className="grid-3" style={{ marginBottom: 'var(--spacing-lg)' }}>
        <div className="card card-orange stat-card-arrow" onClick={() => navigate('/student/tasks')}>
          <div className="stat-card-icon">📌</div>
          <div className="stat-card-value" style={{ color: 'var(--color-warning)' }}>{pendingCount}</div>
          <div className="stat-card-label">待完成任务</div>
        </div>
        <div className="card card-blue stat-card-arrow" onClick={() => navigate('/student/lottery')}>
          <div className="stat-card-icon">⭐</div>
          <div className="stat-card-value" style={{ color: 'var(--color-primary)' }}>{pointBalance}</div>
          <div className="stat-card-label">我的积分</div>
        </div>
        <div className="card card-green stat-card-arrow" onClick={() => navigate('/student/voucher')}>
          <div className="stat-card-icon">💰</div>
          <div className="stat-card-value" style={{ color: 'var(--color-secondary)' }}>{voucherBalance}</div>
