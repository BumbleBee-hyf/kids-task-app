...
export default function LotteryPage() {
  const { user } = useAuth()
  const { pointCost, todayCount, drawBox, spinWheel, refreshCount, config, refreshConfig } =
    useLottery()
  ...
  return (
    <div className={styles.lotteryPage}>
      ...
      {/* 积分统计卡片 */}
      <LotteryCountCard pointBalance={pointBalance} pointCost={pointCost} todayCount={todayCount} />
      ...
      {/* 游戏区域 */}
      {activeGame === 'box' ? (
        <LuckyBox prizes={boxPrizes} onWin={handleBoxWin} disabled={!canDraw} />
      ) : (
        <LuckyWheel prizes={wheelPrizes} onSpinEnd={handleWheelEnd} disabled={!canDraw} />
      )}
