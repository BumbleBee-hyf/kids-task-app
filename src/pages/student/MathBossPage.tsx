...
const BOSS_PROJ_COLORS: Record<string, string> = {
  ...
  pvz_disco: '#FF69B4',
  pvz_football: '#DC143C',
  pvz_gargantuar: '#D2691E',
  pvz_zomboss: '#8B00FF',
  tank_darkbear: '#EF4444',
  tank_xiaozha: '#A855F7',
  tank_sherman: '#60A5FA',
  tank_xiaoban: '#22D3EE',
  tank_xiaolv: '#4ADE80',
...
export default function MathBossPage() {
  ...
  const handleStartHiddenGame = useCallback(
    async (theme: HiddenTheme = 'minecraft') => {
      if (!user) return
  ...
  const renderPlaying = () => {
    ...
    const playerAnimClass =
      animPhase === 'playerAttack' || animPhase === 'shadowStrike' || animPhase === 'risingDragon'
        ? styles.attackRight
        : animPhase === 'playerHurt'
          ? styles.hurt
          : ''
    ...
    const bossAnimClass =
      animPhase === 'bossHurt'
        ? styles.hit
        : animPhase === 'bossAttack'
          ? styles.attackLeft
          : animPhase === 'bossDefeat'
            ? styles.dying
            : animPhase === 'bossEntrance'
              ? styles.entrance
              : ''

    const showPlayerProjectile =
      animPhase === 'playerAttack' || animPhase === 'shadowStrike' || animPhase === 'risingDragon'
    ...
    return (
      <div className={`${styles.battleZone} ${isHidden ? styles.hiddenBattle : ''}`}>
        ...
        <div className={styles.comboBar}>
          <span
            className={`${styles.comboIndicator} ${comboCount >= 5 ? styles.combo5 : comboCount >= 3 ? styles.combo3 : ''}`}
          >
            🔥 {comboCount} 连击
          </span>
          <button className={styles.quitBtn} onClick={handleGoHome}>
            ✕ 退出
          </button>
        </div>
        ...
        <div ref={battleStageRef} className={styles.battleStage}>
          ...
          {comboCount >= 2 && (
            <div
              className={`${styles.comboEdgeGlow} ${comboCount >= 7 ? styles.combo7 : comboCount >= 5 ? styles.combo5 : comboCount >= 3 ? styles.combo3 : ''}`}
            />
          )}
          ...
          {floatingDmg && (
            <div
              ...
              style={{
                left: `${floatingDmg.x}%`,
                top: `${floatingDmg.y}%`,
                color: floatingDmg.color,
              }}
            >
              {floatingDmg.isCritical
                ? String(floatingDmg.value)
                    .split('')
                    .map((digit, i) => (
                      <span
                        key={i}
                        className={styles.floatingDmgDigit}
          ...
          {animPhase === 'bossHurt' && (
            <div
              className={styles.bossHitFlash}
              style={{ background: `radial-gradient(circle, ${bossHitColor}44, transparent 70%)` }}
            />
          )}
          ...
          {hitExplosion && (
            <div
              className={`${styles.hitExplosion} ${hitExplosion.side === 'boss' ? styles.bossSide : styles.playerSide}`}
            >
              <div className={styles.impactRing} style={{ borderColor: hitExplosion.color }} />
              <div
                className={styles.impactFlash}
                style={{ boxShadow: `0 0 20px ${hitExplosion.color}` }}
              />
          ...
          {showCombo && comboCount >= 2 && !isChoosing && (
            <div className={styles.comboDisplay}>
              <div
                className={`${styles.comboText} ${comboCount >= 7 ? styles.comboCrazy : comboCount >= 5 ? styles.superCombo : comboCount >= 3 ? styles.greatCombo : ''} ${comboCount >= 5 ? styles.comboTextShake : ''}`}
              >
                {comboCount >= 7
                  ? '🔥🔥🔥 疯狂连击！'
                  : comboCount >= 5
          ...
          {isSkillAnim && (
            <div
              className={`${styles.skillOverlay} ${animPhase === 'shadowStrike' ? styles.shadowOverlay : styles.dragonOverlay}`}
            >
          ...
          {skillEffects.includes('vignette') && (
            <div
              className={`${styles.skillVignette} ${animPhase === 'shadowStrike' ? styles.shadow : styles.dragon}`}
            />
          )}
          {skillEffects.includes('charge') && (
            <div
              className={`${styles.skillCharge} ${animPhase === 'shadowStrike' ? styles.shadow : styles.dragon}`}
            />
          )}
          {skillEffects.includes('dash') && <div className={styles.dashTrail} />}
          {skillEffects.includes('crossSlash') && (
            <>
              <div
                className={`${styles.slashLine} ${styles.slash1}`}
                style={{
                  background: `linear-gradient(90deg, transparent, #A78BFA, #7C3AED)`,
                  boxShadow: `0 0 10px #A78BFA`,
                }}
              />
              <div
                className={`${styles.slashLine} ${styles.slash2}`}
                style={{
                  background: `linear-gradient(90deg, transparent, #A78BFA, #7C3AED)`,
                  boxShadow: `0 0 10px #A78BFA`,
                }}
          ...
          {skillEffects.includes('launch') && <div className={styles.skillLaunch} />}
          ...
          {/* 玩家弹幕 — 斩击轨迹 */}
          {showPlayerProjectile && !isSkillAnim && <div className={styles.slashTrail} />}

          {/* 技能弹幕 — 能量球 */}
          {showPlayerProjectile && isSkillAnim && (
            <div
              className={`${styles.energyProjectile} ${animPhase === 'shadowStrike' ? styles.shadow : styles.dragon}`}
            />
          )}
            ...
            >
              <span
                style={{
                  ...
                  fontSize: boss.hearts >= 4 ? 20 : boss.hearts >= 3 ? 16 : 14,
                }}
              >
                {boss.attackEffect || '💥'}
              </span>
          ...
          {/* Boss出场VS闪光 */}
          {animPhase === 'bossEntrance' && <div className={styles.vsFlashEffect}>VS</div>}

          {/* 击杀爆炸特效 — 史诗级 */}
          {showDefeatBurst && (
            <div className={styles.defeatBurst}>
              <div
                className={styles.defeatBurstFlash}
                style={{
                  background: `radial-gradient(circle, ${boss.glowColor}88, transparent 60%)`,
                }}
              />
              ...
              {/* 第三波粒子 — 金色余波 */}
              {['#FFD700', '#FBBF24', '#FF8C42', '#F59E0B', '#FFD700', '#FBBF24'].map(
                (color, i) => (
                  <div
                    key={`p3-${i}`}
                    className={styles.defeatParticle}
          ...
          <div className={styles.battleArena}>
            <div className={styles.playerSide}>
                ...
                (() => {
                  ...
                  return (
                    <div className={`${styles.playerAvatar} ${playerAnimClass}`}>
                      <BossAvatar
                        icon={skinBoss.icon}
                        color={skinBoss.color}
                        glowColor={skinBoss.glowColor}
                        size={52}
                      />
                    </div>
                  )
              ...
              {!equippedSkin && (
                <HeroAvatar size={52} className={styles.playerAvatar} animClass={playerAnimClass} />
              )}
            ...
            <div className={styles.bossSide}>
              <div
                className={`${styles.bossAvatar} ${bossAnimClass} ${currentBossHearts === 1 && boss.difficulty === 'hard' ? styles.enragedBoss : ''}`}
              >
                <BossAvatar
                  icon={boss.icon}
                  color={boss.color}
              ...
              <div className={styles.bossNameTag}>
                {boss.name}
                <span
                  className={`${styles.bossDiffTag} ${styles[boss.difficulty]}`}
                  style={{ marginLeft: 4 }}
                >
                  {boss.difficulty === 'easy'
                    ? '简单'
        ...
        {isHidden ? (
          <div className={styles.questionCard}>
            <div className={styles.questionExpression}>
              {question.blank === 'a' ? (
                <span
                  className={`${styles.answerDisplay} ${isCorrect ? styles.correct : ''} ${isWrong ? styles.wrong : ''}`}
                >
                  {inputValue || '?'}
                </span>
              ) : (
            ...
            {isCorrect && !isChoosing && (
              <div className={`${styles.feedbackText} ${styles.correct}`}>
                ✅ 击中了！-{lastDamage}❤️
              </div>
            )}
            {isWrong && (
        ...
        {(answerState === 'input' || isChoosing) && (
          <div className={styles.numpad}>
            ...
            <button
              className={`${styles.numpadBtn} ${styles.delete}`}
              onClick={() => {
                playSound('num_press')
                handleDelete()
              }}
  ...
  const renderResult = () => {
    ...
    return (
      <div className={styles.resultOverlay}>
        <div
          className={`${styles.resultCard} ${isHidden ? styles.hiddenResultCard : ''} ${showUnlockCelebration ? styles.unlockResultCard : ''}`}
        >
          {showUnlockCelebration && (
            <div className={styles.unlockCelebration}>
              ...
              <button
                className={styles.unlockEnterBtn}
                onClick={() => {
                  setShowUnlockCelebration(false)
                  resetGame()
          ...
          {isHidden && (
            <div className={styles.hiddenResultTag}>
              {hiddenTheme === 'pvz'
                ? '🌻 植物大战僵尸'
                : hiddenTheme === 'tank'
                  ? '🔫 超能装甲兵团'
          ...
          <div className={styles.resultStats}>
            <div className={styles.resultStat}>
              <div className={`${styles.resultStatValue} ${styles.green}`}>
                {bossesDefeated}/{bossArray.length}
              </div>
