...
interface MathBossContextType {
  ...
  currentQuestion: { a: number; b: number; operator?: '+' | '-'; answer?: number; result?: number; correctOperator?: '+' | '-'; difficulty: string; mode?: string; blank?: string } | null;
  bossHearts: number[];
  playerHearts: number;
  ...
  startGame: (studentId: string, mode?: GameMode, theme?: HiddenTheme) => Promise<{ success: boolean; error?: string }>;
  /** 仅验证答案，不造成伤害 */
...
export function MathBossProvider({ children }: { children: React.ReactNode }) {
  ...
  const [currentQuestion, setCurrentQuestion] = useState<{ a: number; b: number; operator?: '+' | '-'; answer?: number; result?: number; correctOperator?: '+' | '-'; difficulty: string; mode?: string; blank?: string } | null>(null);
  ...
  const [unlockedSkins, setUnlockedSkins] = useState<{ id: string; bossIcon: string; theme: string; unlockedAt: string }[]>([]);
  ...
  const currentQuestionRef = useRef<{ a: number; b: number; operator?: '+' | '-'; answer?: number; result?: number; correctOperator?: '+' | '-'; difficulty: string; mode?: string; blank?: string } | null>(null);
  ...
  const startGame = useCallback(async (studentId: string, mode: GameMode = 'normal', theme: HiddenTheme = 'minecraft') => {
    try {
  ...
  const checkAnswer = useCallback((answer: number | string): CheckResult => {
    ...
    if (!q) return { isCorrect: false, correctAnswer: 0, pendingCombo: 0, availableSkills: [], gameOver: true };
  ...
  const fetchNextQuestion = useCallback(async () => {
    ...
    if (!boss) return;
    try {
      const result = await mathBossStorage.getQuestion(boss.difficulty, isHidden ? 'hidden' : 'normal');
      if (result.success) {
        setCurrentQuestion(result.question);
        currentQuestionRef.current = result.question;
      }
  ...
  const finishGame = useCallback(async (studentId: string) => {
    ...
    try {
      const result = await mathBossStorage.finish(studentId, defeated, total, correct, mode, hearts, mode === 'hidden' ? hiddenThemeRef.current : undefined);
  ...
  return (
    <MathBossContext.Provider value={{
      phase, gameMode, currentBossIndex, currentQuestion,
      bossHearts, playerHearts, bossesDefeated,
      comboCount, maxCombo, totalQuestions, correctCount, reward,
      todayPlayCount, todayBestScore,
      hiddenUnlocked, hiddenTheme, unlockedSkins, equippedSkin, hiddenTodayPlayCount, hiddenTodayBestScore,
      startGame, checkAnswer, applyAttack, advanceToNextBoss, fetchNextQuestion, finishGame, resetGame,
      refreshStatus, refreshHiddenStatus, refreshSkins, equipSkin,
    }}>
      {children}
