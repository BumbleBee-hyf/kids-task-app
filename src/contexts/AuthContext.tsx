...
interface AuthContextType {
  ...
  register: (username: string, password: string, displayName: string, role: 'student' | 'parent') => Promise<{ success: boolean; error?: string }>;
  logout: () => void;
}
...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  const register = useCallback(async (username: string, password: string, displayName: string, role: 'student' | 'parent') => {
    ...
    const newUser = await userStorage.create({ username, password, displayName: displayName.trim(), role });
    setUser(newUser);
