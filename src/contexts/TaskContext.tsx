...
export function TaskProvider({ children }: { children: React.ReactNode }) {
  ...
  const getTasksByParent = useCallback(
    (parentId: string) => {
  ...
  const getTemplatesByParent = useCallback(
    (parentId: string) => {
      return tasks.filter(
        (t) =>
          t.parentId === parentId &&
          !t.taskDate &&
          (t.taskType === 'daily' || t.taskType === 'periodic'),
      )
    },
    [tasks],
  )
  ...
  const createTask = useCallback(
    async (data: Omit<Task, 'id' | 'createdAt' | 'status'>) => {
  ...
  const approveTask = useCallback(
    async (id: string, rating: TaskRating) => {
